-- Fix Dashboard Statistics Issues
-- Run this in Supabase SQL Editor

-- 1. Fix the issuer_performance view to avoid Cartesian product issues
...
-- 2. Update the dashboard_stats view to fix the diaries_allotted calculation
-- The issue is that it's counting diary_allotments with status 'allotted' 
-- but it should count all diary_allotments regardless of status
...
SELECT 
    ...
    (SELECT COUNT(*) FROM diary_allotments) as diaries_allotted,
